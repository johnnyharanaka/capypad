...
type SiteSettings = {
  ...
  unclaimedCleanupMaxAgeHours: number;
  maxFileBytes: number;
};
type UserItem = {
  id: number;
...
export default function AdminPage() {
  ...
  const [settings, setSettings] = useState<SiteSettings>({
    ...
    unclaimedCleanupMaxAgeHours: 8,
    maxFileBytes: 10485760,
  });
  const [settingsLoading, setSettingsLoading] = useState(false);
  const [settingsSaved, setSettingsSaved] = useState(false);
  const [settingsDaysInput, setSettingsDaysInput] = useState("30");
  const [settingsUnclaimedHoursInput, setSettingsUnclaimedHoursInput] = useState("8");
  ...
  const saveSettings = async (updated: SiteSettings) => {
    ...
    if (res.ok) {
      ...
      setSettingsUnclaimedHoursInput(String(data.unclaimedCleanupMaxAgeHours));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (settingsRes.ok) {
        ...
        setSettingsUnclaimedHoursInput(String(s.unclaimedCleanupMaxAgeHours));
                ...
                <>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between p-4 border border-stone-200 dark:border-stone-700 rounded-xl bg-white dark:bg-stone-800/50">
                      <div className="flex flex-col gap-1">
                        <span className="text-sm font-medium">
                          Hours before unclaimed pads are deleted
                        </span>
                        <span className="text-xs text-stone-400 dark:text-stone-500">
                          Pads with only anonymous edits are removed after this period.
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          min={1}
                          value={settingsUnclaimedHoursInput}
                          onChange={(e) => setSettingsUnclaimedHoursInput(e.target.value)}
                          onBlur={() => {
                            const hours = Math.max(
                              1,
                              parseInt(settingsUnclaimedHoursInput) || 8,
                            );
                            setSettingsUnclaimedHoursInput(String(hours));
                            if (hours !== settings.unclaimedCleanupMaxAgeHours) {
                              const updated = {
                                ...settings,
                                unclaimedCleanupMaxAgeHours: hours,
                              };
                              setSettings(updated);
                              saveSettings(updated);
                            }
