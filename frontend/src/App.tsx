...
import {
  ...
  toggleCode, toggleBulletList, toggleNumberedList, toggleBlockquote,
  setHeading, setAlignment,
...
function renderAlignment(html: string): string {
  ...
  return html.replace(/<(p|h[1-6]|li|div)([^>]*)>\s*\{(center|right|justify)\}/g,
...
const dockBtn = 'w-12 h-12 rounded-xl flex items-center justify-center text-stone-500 dark:text-stone-400 text-base font-semibold transition-all active:scale-90'
const dockBtnActive = 'w-12 h-12 rounded-xl flex items-center justify-center text-stone-800 dark:text-stone-100 bg-stone-200 dark:bg-stone-700 text-base font-semibold transition-all active:scale-90'
const dockItem = 'px-2.5 py-1.5 rounded-lg flex items-center justify-center text-stone-600 dark:text-stone-300 text-sm font-semibold transition-all active:scale-95 hover:bg-stone-200 dark:hover:bg-stone-700'
...
function FloatingDock({ editorView }: { editorView: EditorView | null }) {
  ...
  const groupItems: Record<GroupId, Array<{ label: React.ReactNode; action: () => void; title: string }>> = {
    text: [
      { label: 'B', action: () => toggleBold(editorView), title: 'Bold' },
      { label: 'I', action: () => toggleItalic(editorView), title: 'Italic' },
      { label: 'U', action: () => toggleUnderline(editorView), title: 'Underline' },
      { label: 'S', action: () => toggleStrikethrough(editorView), title: 'Strikethrough' },
    ],
    heading: [
      { label: 'H1', action: () => setHeading(editorView, 1), title: 'Heading 1' },
      { label: 'H2', action: () => setHeading(editorView, 2), title: 'Heading 2' },
      { label: 'H3', action: () => setHeading(editorView, 3), title: 'Heading 3' },
    ],
    list: [
      { label: '•', action: () => toggleBulletList(editorView), title: 'Bullet list' },
      { label: '1.', action: () => toggleNumberedList(editorView), title: 'Numbered list' },
      { label: '</>', action: () => toggleCode(editorView), title: 'Code' },
      { label: '"', action: () => toggleBlockquote(editorView), title: 'Blockquote' },
    ],
    align: [
      { label: <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><rect x="1" y="2" width="12" height="1.5" rx=".5"/><rect x="1" y="5.5" width="8" height="1.5" rx=".5"/><rect x="1" y="9" width="12" height="1.5" rx=".5"/></svg>, action: () => setAlignment(editorView, 'left'), title: 'Left' },
      { label: <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><rect x="1" y="2" width="12" height="1.5" rx=".5"/><rect x="3" y="5.5" width="8" height="1.5" rx=".5"/><rect x="1" y="9" width="12" height="1.5" rx=".5"/></svg>, action: () => setAlignment(editorView, 'center'), title: 'Center' },
      { label: <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><rect x="1" y="2" width="12" height="1.5" rx=".5"/><rect x="5" y="5.5" width="8" height="1.5" rx=".5"/><rect x="1" y="9" width="12" height="1.5" rx=".5"/></svg>, action: () => setAlignment(editorView, 'right'), title: 'Right' },
      { label: <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><rect x="1" y="2" width="12" height="1.5" rx=".5"/><rect x="1" y="5.5" width="12" height="1.5" rx=".5"/><rect x="1" y="9" width="12" height="1.5" rx=".5"/></svg>, action: () => setAlignment(editorView, 'justify'), title: 'Justify' },
    ],
  }

  const sep = <div className="w-px h-5 bg-stone-300/50 dark:bg-stone-600/50 mx-1" />
          ...
          {groupIds.map((id, i) => (
            <div key={id} className="flex items-center gap-1">
              ...
              {groupItems[id].map((item) => (
                <button key={item.title} className={dockItem} onClick={() => act(item.action)} title={item.title}>
                  {item.label}
                </button>
              ))}
            </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-stone-50 dark:bg-stone-900 text-stone-800 dark:text-stone-100">
      <header className="sticky top-0 z-10 bg-stone-50/80 dark:bg-stone-900/80 backdrop-blur-md px-12 py-4 flex items-center shrink-0">
        <a href="/" className="no-underline hover:opacity-70 transition-opacity text-sm shrink-0">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[11px] text-stone-400 dark:text-stone-500 tabular-nums mr-1">
            <span className="hidden sm:inline">{words} words · {chars} chars</span>
            <span className="sm:hidden">{words}w · {chars}c</span>
          </span>
